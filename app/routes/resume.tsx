...
const Resume = () => {
    ...
    useEffect(() => {
        // Track URLs created in this effect so we can revoke them on cleanup
        ...
        const loadResume = async () => {
            ...
            blobUrls.push(newResumeUrl);
            setResumeUrl(newResumeUrl);

            const imageBlob = await fs.read(data.imagePath);
            if (!imageBlob) return;
            const newImageUrl = URL.createObjectURL(imageBlob);
            blobUrls.push(newImageUrl);
        ...
        // Revoke all object URLs on cleanup to prevent memory leaks
        return () => {
